...
const addToFavorite = async (item) => {
  try {
    if (!item.isFavorite) {
      ...
      const { data } = await axios.post(`https://7d3fdcfb4b5e9f87.mokky.dev/favorites`, obj) // Отправляем POST-запрос для добавления элемента в избранное
      ...
      await axios.delete(`https://7d3fdcfb4b5e9f87.mokky.dev/favorites/${item.favoriteId}`) // Отправляем DELETE-запрос для удаления элемента из избранного
...
const fetchFavorites = async () => {
  try {
    ...
    const { data: favorites } = await axios.get(`https://7d3fdcfb4b5e9f87.mokky.dev/favorites`)
...
const fetchItems = async () => {
  try {
    ...
    const { data } = await axios.get(`https://7d3fdcfb4b5e9f87.mokky.dev/items`, { params })
...
<template>
  <div class="flex justify-between items-center">
    <h2 class="text-3xl font-bold mb-8">Вся косметика</h2>
